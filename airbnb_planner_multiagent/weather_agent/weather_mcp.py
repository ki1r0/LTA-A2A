...
async def get_forecast_by_city(city: str, state: str) -> str:
    ...
    for q in queries:
        try:
            location = geolocator.geocode(q, timeout=GEOCODE_TIMEOUT)
            if location:
                break
    ...
    if location is None:
        if last_error:
            return f"Could not find coordinates for '{city_name}, {state_code}'. Last error: {last_error}"
        else:
