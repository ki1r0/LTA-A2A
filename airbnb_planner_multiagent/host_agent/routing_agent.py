...
class RoutingAgent:
    ...
    def create_agent(self) -> Agent:
        ...
        model_id = os.getenv("GOOGLE_GENAI_MODEL")
        api_key = os.getenv("GOOGLE_API_KEY")

        if not api_key:
            raise ValueError("GOOGLE_API_KEY environment variable is not set")
        
        print(f"Using Google Generative AI model: {model_id}")
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        **Final Travel Plan Directive:**
    
        Once you have successfully gathered responses from both the weather and Airbnb agents, call the `generate_travel_plan` tool to combine the weather and accommodation summaries into a complete plan. Ensure both `weather_summary` and `airbnb_options` are present in memory before calling this tool.
        **Agent Roster:**
    ...
    async def send_message(
        ...
        if isinstance(send_response.root, SendMessageSuccessResponse) and isinstance(send_response.root.result, Task):
            ...
            if artifacts:
                message_parts = artifacts[0].parts
            else:
                message_parts = []
            final_response = "\n".join(convert_parts(message_parts, tool_context))

        if agent_name.lower().startswith("weather"):
            tool_context.state["weather_summary"] = final_response
        elif agent_name.lower().startswith("airbnb"):
            tool_context.state["airbnb_options"] = final_response


        return send_response.root.result

    async def generate_travel_plan(self, tool_context: ToolContext):
        """Combine weather and Airbnb information into a travel plan summary."""
        weather = tool_context.state.get("weather_summary", "")
        airbnb = tool_context.state.get("airbnb_options", "")
        if not weather or not airbnb:
            return {
                "is_task_complete": False,
                "require_user_input": True,
                "content": "Missing weather or Airbnb information. Cannot generate travel plan."
            }

        return {
            "is_task_complete": True,
            "require_user_input": False,
            "content": f"""Here is your travel plan:
